...
export const schemeFundManagersRelations = relations(scheme_fund_managers, ({ one }) => ({
    scheme: one(schemes, {
        fields: [scheme_fund_managers.schemeId],
        references: [schemes.id],
    }),
    fundManager: one(fund_managers, {
        fields: [scheme_fund_managers.fundManagerId],
        references: [fund_managers.id],
    }),
}));
