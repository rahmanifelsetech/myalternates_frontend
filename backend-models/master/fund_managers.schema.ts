import { pgTable, uuid, varchar, text, boolean, integer, timestamp, numeric } from "drizzle-orm/pg-core";
import { InferInsertModel, InferSelectModel, relations } from "drizzle-orm";
import { amcs } from "../amc/amc.schema";
import { scheme_fund_managers } from "../amc/scheme_fund_managers.schema";

export const fund_managers = pgTable("fund_managers", {
    id: uuid("id").defaultRandom().primaryKey(),
    name: varchar("name", { length: 255 }).notNull(),
    code: varchar("code", { length: 50 }),
    designation: varchar("designation", { length: 255 }),
    amcId: uuid("amc_id").references(() => amcs.id),
    aum: numeric("aum", { precision: 20, scale: 2 }),
    profilePicture: text("profile_picture"),
    linkedinUrl: text("linkedin_url"),
    about: text("about"),
    experience: text("experience"),
    fundManagerCreative: text("fund_manager_creative"),
    isFeatured: boolean("is_featured").default(false),
    priorityOrder: integer("priority_order").default(0),
    isActive: boolean('is_active').default(true).notNull(),
    createdAt: timestamp("created_at").defaultNow(),
    updatedAt: timestamp("updated_at").defaultNow(),
});

export const fundManagersRelations = relations(fund_managers, ({ one, many }) => ({
    amc: one(amcs, {
        fields: [fund_managers.amcId],
        references: [amcs.id],
    }),
    schemes: many(scheme_fund_managers),
}));

export type FundManager = InferSelectModel<typeof fund_managers>;
export type NewFundManager = InferInsertModel<typeof fund_managers>;